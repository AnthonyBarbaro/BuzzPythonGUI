...
def extract_strain_type(product_name: str):
    """Extract strain type (S, H, I) from product name."""
...
def extract_product_details(product_name: str):
    """
    Extract product weight (like 1.6G, 1G) and sub-type (HH or IN) from product name.
    Adjust regex/logic based on the actual naming conventions of the products.
    """
    if not isinstance(product_name, str):
        return "", ""
    ...
    # Extract weight (looking for patterns like '1.6G', '1G', '0.5G')
    weight_match = re.search(r'(\d+(\.\d+)?)G', name_upper)
    weight = weight_match.group(0) if weight_match else ""

    # Identify sub-type: HH or IN if they appear as separate words
    sub_type = ""
...
def is_empty_or_numbers(val):
    """Check if a string is empty or consists only of numbers (no letters)."""
...
def format_excel_file(filename: str):
    """Format the final Excel file for better readability."""
    ...
    for ws in wb.worksheets:
        ...
        # Bold and center headers + header fill
        ...
        # Ensure 'Available' column is at least 20 chars wide
        available_col = None
        category_col = None
        strain_col = None
        product_col = None
        ...
        # Insert category headers if category_col is found
        if category_col:
            ...
            # Insert rows for category headers
...
def process_file(file_path, output_directory, selected_brands):
    ...
    # Remove rows with empty or numeric-only Product names
    if 'Product' in available_data.columns:
        available_data = available_data[~available_data['Product'].apply(is_empty_or_numbers)]

    # Sort data by Category, Strain_Type, Product_Weight, Product_SubType, Product
    sort_cols = []
    ...
    if brand_exists:
        # Group by brand if Brand column is available
        if available_data.empty:
            # No data after filtering, still output an empty file
...
def process_files(input_directory, output_directory, selected_brands):
    ...
    # Create or open summary
    if os.path.exists(summary_file):
        summary_df = pd.read_csv(summary_file)
    else:
        summary_df = pd.DataFrame(columns=['File', 'Status'])

    # Initialize or create unavailable file
    ...
    # After processing, delete done.csv and unavailable.csv files
    if os.path.exists(summary_file):
...
def get_all_brands(input_directory):
    """Scan all CSV files in input_directory and return a unique list of brands (if Brand column exists)."""
    brands = set()
    brand_found = False
    for filename in os.listdir(input_directory):
        if filename.endswith('.csv'):
            file_path = os.path.join(input_directory, filename)
            try:
                df = pd.read_csv(file_path, nrows=50)  # nrows=50 for faster scanning
                ...
                pass
    if not brand_found:
        # No brand column in any file
        return None
    return sorted(list(brands))

def save_config(input_dir, output_dir):
    """Save last used directories to config file."""
    with open(CONFIG_FILE, 'w', encoding='utf-8') as f:
        f.write(f"{input_dir}\n")
        f.write(f"{output_dir}\n")

def load_config():
    """Load directories from config file if exists."""
...
def auto_detect_dirs():
    """Auto-detect 'files' and 'done' directories if present in the script's directory."""
...
class App:
    def __init__(self, master):
        self.master = master
        master.title("Cannabis Report Generator")
        ...
        if not i_dir or not o_dir:
            # If not loaded from config, try auto-detecting
            i_dir, o_dir = auto_detect_dirs()

        # If still None, just leave them blank
        if i_dir and o_dir:
        ...
        # Frame for brand selection
        brand_frame = tk.Frame(master)
    ...
    def load_brands(self):
        ...
        try:
            ...
            if brands is None:
                ...
                self.brand_listbox.insert(tk.END, "No brand column found in any file.")
                self.brand_listbox.configure(state='disabled')
            else:
                if len(brands) == 0:
                    # Brand column found but no brand values
                    self.brand_listbox.insert(tk.END, "No brands found. You can still run the report.")
                    self.brand_listbox.configure(state='disabled')
                else:
                    # Populate listbox with brands
                    for b in brands:
    ...
    def select_all_brands(self):
        # Select all items in the listbox if it is enabled and has items
        if self.brand_listbox['state'] == 'normal':
            if self.brand_listbox.size() > 0:
                first_item = self.brand_listbox.get(0)
                if "No brands found" in first_item or "No brand column found" in first_item:
